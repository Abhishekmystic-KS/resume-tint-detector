...
/** Same browser-side call, but comparing the resume against a job description. */
export async function matchWithUserKey(
  apiKey: string,
  text: string,
  jd: string,
): Promise<{ ok: boolean; markdown?: string; error?: string }> {
  ...
  try {
    res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        contents: [{ role: "user", parts: [{ text: buildMatchPrompt(text, jd) }] }],
      }),
    });
  } catch {
    return { ok: false, error: "Couldn't reach Google. Check your connection and try again." };
